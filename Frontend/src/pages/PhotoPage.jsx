...
export default function PhotoPage() {
    ...
    const mockAllPhotos = useMemo(() => Array.from({ length: 20 }, (_, i) => ({
        id: `img${i + 1}`,
    ...
    // --- State Management ---
    // activeTab should be managed by the routing, but kept here for internal component logic
    // We'll determine the active tab based on the current URL path in a real app,
    // but for this standalone component, we can default it.
    const [activeTab, setActiveTab] = useState('Photos');
    const [searchQuery, setSearchQuery] = useState('');
    const [profileUser, setProfileUser] = useState(mockProfileUser);
    const [allPhotoGallery] = useState(mockAllPhotos); // All photos available
    const [photosLoadedCount, setPhotosLoadedCount] = useState(PHOTOS_PER_LOAD);
    ...
    // This state should hold the photos currently visible on the page
    const [displayedPhotos, setDisplayedPhotos] = useState([]);

    // Simulate fetching data on mount (replace with actual API calls)
    useEffect(() => {
        // In a real app, you'd fetch profile data and initial photos here
        // For now, we just set the initial displayed photos
        setDisplayedPhotos(mockAllPhotos.slice(0, PHOTOS_PER_LOAD));
        setPhotosLoadedCount(PHOTOS_PER_LOAD); // Ensure count is reset on mount
    }, [mockAllPhotos]);
                ...
                <div className="flex overflow-x-auto whitespace-nowrap space-x-6 border-b border-slate-700 mb-6 custom-scrollbar" role="tablist">
                    {['Posts', 'About', 'Photos', 'Videos', 'Friends'].map((tab) => (
                        <motion.button
                            key={tab}
                            onClick={() => {
                                // Use navigate for external pages, set activeTab for the current page
                                if (tab === 'Posts') {
                                    navigate('/profile-feed'); // Example route for posts
                                } else if (tab === 'About') {
                                    navigate('/profile/about'); // Example route for about
                                } else if (tab === 'Videos') {
                                    navigate('/profile/videos'); // Example route for videos
                                } else if (tab === 'Friends') {
                                    navigate('/profile/friends'); // Example route for friends
                                } else {
                                    // 'Photos' tab: stay on this page and ensure it's active
                                    setActiveTab('Photos');
                                    // You might also want to navigate to /profile/photos if not already there
                                    if (window.location.pathname !== '/profile/photos') {
                            ...
                            role="tab"
                            // If you use a real routing solution, you might also use a Link component here
                            // Or determine activeTab based on current URL like:
                            // className={location.pathname === '/profile/photos' ? 'active' : ''}
                        >
                            {tab}
                            {activeTab === tab && (
                                <motion.span
